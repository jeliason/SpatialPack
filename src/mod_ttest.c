...
{ /* Moran's I */
  int pos;
  double dx, dy, distance, dummy, sx[dims->nclass], sy[dims->nclass], xbar, xvar, ybar, yvar, wts[dims->nclass];
  ...
  for (int k = 0; k < dims->nclass; k++) {
    index[k] = (sx[k] / wts[k]) / xvar;
    index[k + dims->nclass] = (sy[k] / wts[k]) / yvar;
    card[k] = wts[k];
  }
}
